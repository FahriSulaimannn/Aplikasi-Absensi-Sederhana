...
try:
    from service.divisi import select_divisi
    from service.karyawan import (
        hapus_karyawan,
        tambah_karyawan,
        tampil_karyawan,
        update_karyawan,
...
class HalamanKaryawan(ttk.Frame):
    ...
    def build_ui(self):
        ...
        ttk.Label(header, text="👥 Kelola Karyawan", font=("Arial", 14, "bold")).pack(
            side="left", padx=15
        )
        ...
        ttk.Button(
            aksi, text="➕ Tambah", command=lambda: self.buka_form(is_edit=False)
        ).pack(side="left", padx=2)
        ttk.Button(
            aksi, text="✏️ Edit", command=lambda: self.buka_form(is_edit=True)
        ).pack(side="left", padx=2)
        ttk.Button(aksi, text="🗑️ Hapus", command=self.hapus_data).pack(
            side="left", padx=2
        )
    ...
    def muat_data(self):
        ...
        try:
            self.list_divisi = [{"id": r[0], "nama": r[1]} for r in select_divisi()]
        except Exception:
            pass
        ...
        for r in tampil_karyawan():
            ...
            self.tree.insert(
                ...
                "end",
                iid=str(r[0]),
                values=(r[0], r[1], r[4], r[2], r[3], r[5].capitalize(), r[7] or "—"),
            )
    ...
    def get_id(self):
        ...
        if not sel:
            messagebox.showwarning(
                "Pilih Data", "Pilih karyawan di tabel terlebih dahulu!"
            )
    ...
    def buka_form(self, is_edit):
        ...
        if is_edit and not id_karyawan:
            return
        ...
        for i, field in enumerate(fields):
            ...
            if field == "Divisi":
                ent = ttk.Combobox(
                    pop, values=nama_divisi_list, state="readonly", width=23
                )
                if nama_divisi_list:
                    ent.current(0)  # Set default item pertama
            elif field == "Posisi":
        ...
        if is_edit:
            data_lama = next(
                (r for r in tampil_karyawan() if r[0] == id_karyawan), None
            )
            if data_lama:
        ...
        def simpan():
            ...
            id_div = next(
                (d["id"] for d in self.list_divisi if d["nama"] == v["Divisi"]), None
            )
            if not id_div:
            ...
            try:
                ...
                messagebox.showerror(
                    "Error DB", f"Gagal menyimpan data: {e}", parent=pop
                )

        ttk.Button(pop, text="Simpan", command=simpan).grid(
