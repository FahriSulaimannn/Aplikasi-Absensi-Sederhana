...
class HalamanDivisi(ttk.Frame):
    ...
    def build_ui(self):
        ...
        ttk.Label(header, text="🏬 Kelola Divisi", font=("Arial", 14, "bold")).pack(
            side="left", padx=15
        )
    ...
    def get_id(self):
        ...
        if not sel:
            messagebox.showwarning(
                "Pilih Data", "Pilih divisi di tabel terlebih dahulu!"
            )
    ...
    def buka_form(self, is_edit):
        ...
        ttk.Label(pop, text="Nama Divisi").grid(
            row=0, column=0, sticky="w", padx=15, pady=20
        )
        ...
        if is_edit:
            try:
                data_lama = next(
                    (r for r in select_divisi() if r[0] == id_divisi), None
                )
                if data_lama:
        ...
        def simpan():
            ...
            if not nama_baru:
                messagebox.showwarning(
                    "Peringatan", "Nama divisi tidak boleh kosong!", parent=pop
                )
                return
    ...
    def hapus_data(self):
        ...
        if messagebox.askyesno(
            "Hapus", f"Yakin ingin menghapus divisi '{nama_divisi}'?"
        ):
            try:
                delete_divisi(id_divisi)
                self.muat_data()
                messagebox.showinfo(
                    "Berhasil", f"Divisi '{nama_divisi}' berhasil dihapus."
                )
            except Exception as e:
